...
/**
 * The HomeExtender command is responsible for homing the robot's extender
 * mechanism.
 * This typically involves driving the extender inwards at a set voltage until a
 * stall
 * condition (indicated by high current draw) is met, then resetting the
 * encoders
...
public class HomeExtender extends Command {
  ...
  public void initialize() {
    if (GlobalConstants.IS_BENCHTOP) {
      m_extender.setIsHomed(true);
      System.out.println("Benchtop detected: Skipping physical homing.");
    } else {
   ...
   * This method continuously sets a homing voltage to Ddrive the extender inward.
   */
  @Override
  public void execute() {
    if (!GlobalConstants.IS_BENCHTOP) {
   ...
   * 
   * @param interrupted True if the command was interrupted by another, false
   *                    otherwise.
   */
  @Override
   ...
   * detect a current draw exceeding the maximum homing voltage, indicating a
   * stall.
   * 
